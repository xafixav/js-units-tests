...
const sumMenu = ({ menuKeys, menu, item }) => {
  let value = 0;
  for (let consumableType of menuKeys) {
    let valueOfItem = menu[consumableType][item];
    if (typeof valueOfItem === 'number') {
      value += valueOfItem;
    }
  }
  return value;
...
const createMenu = (obj) => {
  const result = {
    fetchMenu: () => obj,
    consumption: [],
    order: (food) => { result.consumption.push(food); },
    pay: () => {
      let totalValue = 0;
